...
def minandmax(req):
    ...
    filename = 'docs/escenario1/accesibility_ij_resumen/{}minandmax.json'.format(subfolder)
    try:
        ...
        return HttpResponse(json.dumps({'err': sys.exc_info()}), content_type="application/json")
...
def totalZonasPorTiempo(req):
    ...
    filename = 'docs/escenario1/accesibility_ij_resumen/totalZonaPorTiempo/t_{}.json'.format(tiempo)
    if modo == 'agregado':
        totalData = {'value': 0}
    else:
        totalData = []
    ...
    for zona in zonas:
        try:
            ...
            return HttpResponse(json.dumps({'error': 'revisar zonas'}), content_type="application/json")
        if modo == 'agregado':
...
def tpctmcar(req):
    ...
    for tiempo in tiempos:
        try:
            ...
            return HttpResponse(json.dumps({'error': 'revisar tiempos'}), content_type="application/json")
        if modo == 'agregado':
...
def newTimeline(req):
    ...
    try:
        for zona in zonas:
            filename = 'docs/escenario1/accesibility_ij_resumen/timelines/timeline_{}.json'.format(zona)
            with open(filename, 'r') as file:
                file_data = file.read()
                data = json.loads(file_data)
                print("data")
...
def heatmap(req):
    ...
    try:
        ...
        for time in range(mintime, maxtime + 1):
            filename = 'docs/escenario1/accesibility_ij_resumen/heatmap/heatmap_t{}.json'.format(time)
            with open(filename, 'r') as file:
                file_data = file.read()
        ...
        return HttpResponse(json.dumps([d for d in data if d is not None]), content_type="application/json")
    except:
        print('error - error: ', sys.exc_info())
        return HttpResponse(json.dumps({'err':  sys.exc_info()}), content_type="application/json")
...
def waprfile(request):
    escenarios = request.GET.get('escenarios', 'escenario1').split('$')
    nombres = request.GET.get('nombres', 'slow').split('$')
    ...
    if zonasRangos != None:
        ...
        for zonasRango in zonasRangos.split("$"):
            if ("-" in zonasRango) and (len(zonasRango.split("-")) == 2):
                ...
                try:
                    zonas += [i for i in range(min(int(zonaMin), int(zonaMax) + 1), max(int(zonaMin), int(zonaMax) + 1))]
                except:
                    continue
        ...
        for escenario in escenarios:
            for nombre in nombres:
                for zona in zonas:
                    for modo in modos:
                        for tipo in tipos:
                            file_location = 'docs/{0}/{1}[{2},{3},{4}].json'.format(escenario, nombre, zona, modo, tipo)
    ...
    if len(data) == 0:
        ...
        if csv != 'false':
            ...
            response = HttpResponse(acc_ij_df.to_csv(index=None), content_type='text/csv')
...
def timelineControlData(req):
    ...
    onlyfiles = [f for f in listdir('docs/escenario{0}/'.format(escenario)) if isfile(join('docs/escenario{0}/'.format(escenario), f)) and nombre in f and validatePath(f, attrs)]
    ...
    try:
        return HttpResponse(dat.to_json(orient='records'), content_type="application/json")
    except:
